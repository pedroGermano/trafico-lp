...
import {HiOutlineMinus, HiOutlinePlus, HiOutlinesMinus } from 'react-icons/hi'
  ...
  return (
    <motion.div variants={fadeInRight} className='max-w-[550px]'>
      <div onClick={() => {
       ...
       className="drop-shadow-primary bg-white
       cursor-pointer rounded-[10px] h-[90px] px-[35px] flex items-center "
      >
        <div className='w-full flex justify-between items-center'>
          <p className='lead font-medium leading-snug max-w-[400px]'>{question}</p>
          <div className='transition-all duration-500'>
            {isOpen ? (
              <motion.div initial="initial" animate={{
                rotate: 180,
              }}>
